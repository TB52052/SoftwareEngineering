document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('/profile/data').then(response => {
        if (response.status === 500) { return showMessage('Error fetching user data', true); }
...
function submitNameEditRquest() {
    ...
    fetch('/profile/name', {
        ...
        if (response.status === 403) { return showMessage('Incorrect Password', true); }
        if (response.status === 500) { return showMessage('Error updating name', true); }
    });
    eraseInputs();
}
...
function submitPasswordEditRequest() {
    ...
    if (newPassword !== confirmPassword) { return showMessage('Passwords do not match', trie); }
    ...
    fetch('/profile/password', {
        ...
        if (response.status === 200) { return showMessage('Password updated', false); }
        if (response.status === 403) { return showMessage('Incorrect Password', true); }
        if (response.status === 500) { return showMessage('Error updating password', true); }
    });
    eraseInputs();
}
...
function deleteAccount() {
    ...
    fetch('/profile/delete', {
        ...
        eraseInputs();
        if (response.status === 403) { return showMessage('Incorrect Password', true); }
        if (response.status === 500) { return showMessage('Error deleting account', true);}
    });
}
